...
def alcohol_consumption_label(value):
    if value == 0:
        return 'No'
    elif 0 < value <= 10:
        return 'Little bit'
...
alcohol_label = alcohol_consumption_label(alcohol_consumption)
st.write('Alcohol Consumption:', alcohol_label)

sleep_quality = st.number_input('Sleep Quality', min_value=0.0, max_value=10.0, value=7.0)
family_history_alzheimers = st.selectbox('Family History of Alzheimer\'s', [0, 1], format_func=lambda x: 'No' if x == 0 else 'Yes')
...
behavioral_problems = st.selectbox('Behavioral Problems', [0, 1], format_func=lambda x: 'No' if x == 0 else 'Yes')
...
personality_changes = st.selectbox('Personality Changes', [0, 1], format_func=lambda x: 'No' if x == 0 else 'Yes')
difficulty_completing_tasks = st.selectbox('Difficulty Completing Tasks', [0, 1], format_func=lambda x: 'No' if x == 0 else 'Yes')
